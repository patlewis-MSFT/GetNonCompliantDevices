...
####################################################
Function Get-DeviceComplianceStatuses()
{

    <#
.SYNOPSIS
This function is used to get device compliance policy status from the Graph API REST interface
.DESCRIPTION
The function connects to the Graph API Interface and gets a device compliance policy status
.EXAMPLE
Get-DeviceComplianceStatuses -id $id
Returns any device compliance policy assignment configured in Intune
.NOTES
NAME: Get-DeviceComplianceStatuses
#>
    [cmdletbinding()]
    param
    (
        [Parameter(Mandatory = $true, HelpMessage = "Enter id (guid) for the Device Compliance Policy you want to check")]
        $id
    )

    $graphApiVersion = "Beta"
    $DCP_resource = "deviceManagement/deviceConfigurations/$id/deviceStatuses"
...
foreach ($DCP in $DCPs)
{
    ...
    #For each of the policies look at the device status:
    #   https://graph.microsoft.com/v1.0/deviceManagement/deviceConfigurations/<GUID>/deviceStatuses
    ...
    if ($DCPA)
    {
        ...
        if ($DCPA.count -gt 1)
        {
            ...
            {
                Write-Host (Get-AADGroup -id $group.target.GroupId).displayName -f Cyan
            }
        }
        else
        {
            Write-Host (Get-AADGroup -id $DCPA.target.GroupId).displayName -f Cyan
        }
    }
    Write-Host

    # Now get the device compliance
    $DeviceStatuses = Get-DeviceComplianceStatuses -id $id

    Write-Host "Searching for nonCompliant devices" -f Red
    Write-Host

    foreach ($DeviceStatus in $DeviceStatuses)
    {
        if ($DeviceStatus.status -eq "nonCompliant")
        {
            # Add the device to the policy non comp array
            $PolicyNonCompliantDevices += $DeviceStatus
            Write-Host "deviceDisplayName: " $DeviceStatus.deviceDisplayName -f Green
            Write-Host "id:                " $DeviceStatus.id -f Green
            Write-Host "userName:          " $DeviceStatus.userName -f Green
            Write-Host "Status:            " $DeviceStatus.status -f Green
        }
    }   
}
...
Write-Host
Write-Host
Write-Host "Devices may also be nonCompliant due to a Compliant Policy not being assigned"
Write-Host "Now checking for managed Devices not in above list that are also nonCompliant"
Write-Host "-----------------------------------------------------------------------------"
